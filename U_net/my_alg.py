...
def build_model_32_32():
    ...
    block_0 = keras.layers.Conv2D(
        32, 3, padding='same', activation=acf)(block_0_in)
    block_0 = keras.layers.Conv2D(
        32, 3, padding='same', activation=acf)(block_0)
    ...
    block_1 = keras.layers.Conv2D(
        64, 3, padding='same', activation=acf)(block_1_in)
    block_1 = keras.layers.Conv2D(
        64, 3, padding='same', activation=acf)(block_1)
    ...
    block_2 = keras.layers.Conv2D(
        128, 3, padding='same', activation=acf)(block_2_in)
    block_2 = keras.layers.Conv2D(
        128, 3, padding='same', activation=acf)(block_2)
    ...
    block_3 = keras.layers.Conv2D(
        256, 3, padding='same', activation=acf)(block_3_in)
    block_3 = keras.layers.Conv2D(
        256, 3, padding='same', activation=acf)(block_3)
    block_3 = keras.layers.BatchNormalization()(block_3)
    block_3 = keras.layers.add([block_3, block_3_in])
    # out 4 x 4 @128

    up_block_0 = keras.layers.Conv2DTranspose(
        128, 2, 2, activation=acf)(block_3)
    ...
    up_block_0 = keras.layers.Conv2D(
        128, 3, padding='same', activation=acf)(up_block_0)
    up_block_0_in = keras.layers.BatchNormalization()(up_block_0)

    up_block_0 = keras.layers.Conv2D(
        128, 3, padding='same', activation=acf)(up_block_0_in)
    up_block_0 = keras.layers.Conv2D(
        128, 3, padding='same', activation=acf)(up_block_0)
    up_block_0 = keras.layers.BatchNormalization()(up_block_0)
    ...
    up_block_1 = keras.layers.Conv2DTranspose(
        64, 2, 2, activation=acf)(up_block_0)
    up_block_1 = keras.layers.BatchNormalization()(up_block_1)
    # out 16 x 16 @ 32

    # skip connection
    up_block_1 = keras.layers.concatenate([up_block_1, block_1])
    up_block_1 = keras.layers.Conv2D(
        64, 3, padding='same', activation=acf)(up_block_1)
    up_block_1_in = keras.layers.BatchNormalization()(up_block_1)

    up_block_1 = keras.layers.Conv2D(
        64, 3, padding='same', activation=acf)(up_block_1_in)
    up_block_1 = keras.layers.Conv2D(
        64, 3, padding='same', activation=acf)(up_block_1)
    up_block_1 = keras.layers.BatchNormalization()(up_block_1)
    ...
    up_block_2 = keras.layers.Conv2DTranspose(
        32, 2, 2, activation=acf)(up_block_1)
    up_block_2 = keras.layers.concatenate([up_block_2, block_0])
    # out 32 x 32 @ 32
    up_block_2 = keras.layers.Conv2D(
        32, 3, padding='same', activation=acf)(up_block_2)
    up_block_2_in = keras.layers.BatchNormalization()(up_block_2)

    up_block_2 = keras.layers.Conv2D(
        32, 3, padding='same', activation=acf)(up_block_2_in)
    up_block_2 = keras.layers.Conv2D(
        32, 3, padding='same', activation=acf)(up_block_2)
    ...
    # 1 x 1 conv
    out_img = keras.layers.Conv2D(
        64, 5, padding='same', activation=acf)(up_block_2)
    out_img = keras.layers.BatchNormalization()(out_img)
    out_img = keras.layers.Conv2D(
        1, 1, activation=keras.activations.sigmoid)(out_img)
...
def build_model_128_3layer():
    ...
    block_0_in = keras.layers.Conv2D(
        32, 3, padding='same', activation=acf)(img_input)
    block_0 = keras.layers.BatchNormalization()(block_0_in)
    ...
    # 1 x 1 conv
    out_img = keras.layers.Conv2D(
        3, 3, padding='same', activation=keras.activations.sigmoid)(up_block_2)
    model = keras.Model(img_input, out_img)
...
def build_model_128():
    ...
    # 1 x 1 conv
    out_img = keras.layers.Conv2D(
        1, 3, padding='same', activation=keras.activations.sigmoid)(up_block_2)
    model = keras.Model(img_input, out_img)
...
def show_some_results(x, y, y_pred, num_rows, indexes=None, img_size=5, resize_shape=(128, 128)):
    ...
    fig, axs = plt.subplots(num_rows, 4, figsize=(
        4 * img_size, num_rows * img_size))
...
def show_some_results_3layer(x, y, y_pred, num_rows, indexes=None, img_size=5, resize_shape=(128, 128)):
    ...
    fig, axs = plt.subplots(num_rows, 6, figsize=(
        6 * img_size, num_rows * img_size))
    for row in range(num_rows):
        sid = indexes[row]
        ax = axs[row]
        ax[0].imshow(
...
def show_random_sampels(imgs, num_row, num_col, idx=None, img_size=5, resize_shape=(128, 128)):
    ...
    fig, axs = plt.subplots(num_row, num_col, squeeze=False,  figsize=(
        num_col * img_size, num_row * img_size))
    ...
    for r in range(num_row):
        for c in range(num_col):
            ...
            ax = axs[r, c]
            ax.axis('off')
            ax.imshow(skt.resize(norm_img(img), output_shape=resize_shape,
                      anti_aliasing=True, mode='constant', cval=0))
    return fig
...
def resize_data_set(x, out_shape=(128, 128), add_dim=True):
    ...
    out = []
    for img in x:
        img = skt.resize(norm_img(img), output_shape=out_shape,
                         anti_aliasing=True, mode='constant', cval=0).astype(np.float32)
        out.append(img)
...
def ave_cross_en(y_true, y_pred):
    loss = (keras.losses.kullback_leibler_divergence(
        y_true, y_pred) + keras.losses.kld(1 - y_true, 1 - y_pred))
    return loss
...
def test_tp_minst():
    ...
    model.compile(optimizer=keras.optimizers.Adam(
        3e-4), loss=ave_cross_en, metrics=['mae'])
    model.fit(mnist_b, mnist_x, batch_size=16,
